...
| 2026-07-30 | **Feature:** หลัง insert/update/delete อยู่หน้าจอเดิม ไม่ redirect ออกจาก module + แสดง success message + fix rollBack error + fix column name |
| 2026-07-30 | **Bugfix:** แก้ product edit.php ไม่ refresh ข้อมูลหลัง save (re-fetch $product หลัง UPDATE) |
| 2026-07-30 | **Feature:** เพิ่มฟังก์ชันลบ Customer, Prospect + fix re-fetch หลัง save edit |
...
| 2026-07-31 | **Fix:** บรรทัดรายละเอียดใต้รายการบนใบ Quotation แสดง `products.description` **ค่าปัจจุบัน** เป็นหลัก (ไม่ใช่ค่าที่บันทึกตอนสร้าง) + JOIN ด้วย product_code จาก item_name สำหรับใบเก่าที่ product_id เป็น NULL |
